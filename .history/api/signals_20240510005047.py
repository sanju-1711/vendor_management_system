...
def on_time_delivery_rate(sender, instance, **kwargs):
    ...
    and store it in vendor model if no such row, 
    or update it
    '''
...
def avg_response_time(sender, instance, **kwargs):
    if instance.status == 'Completed' and instance.acknowledgement_date is not None:
        ...
        # Update Vendor with the calculated average response time
            ...
            obj = Vendor.objects.get(vendor_code = instance.vendor.vendor_code)historical_data.average_response_time = avg_response_time
...
def update_fulfillment_rate(sender, instance, **kwargs):
    if instance.status == 'Completed' and instance.acknowledgement_date is not None:
        ...
        try:
            ...
            # Update Historical_Performance with the calculated fulfillment time
            historical_data, _ = Historical_Performance.objects.get_or_create(vendor=instance.vendor)
            historical_data.average_response_time = fulfillment_rate
            historical_data.save()
