...
class UpdatePurchaseAcknowledgement(APIView):
    '''
    to update acknowledgement_date for a given order
    '''
    ...
    def put(self, request, pk):
        order = self.get_object(pk)
        serializer = PurchaseOrderDetails(instance = order, data = request.data, partial = True)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)        
...
class HistoricalPerformance(APIView):
    '''
    to fetch historical data for following things for any given vendor on any value update: 
        - On-Time delivery rate
        - Quality Rating
        - Response Time
        - Fulfillment Rate
    ...
    def get_object(self, pk):
        #Returns a historical_performance object instance
        try:
            return Historical_Performance.objects.get(vendor = pk)
        except:
            raise Http404
